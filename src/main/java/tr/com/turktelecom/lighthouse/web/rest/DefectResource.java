...
//                JsonNode root = mapper.readTree(search);
//                Iterator<JsonNode> elements = root.elements();
//                if (elements.hasNext()) {
//                    JsonNode searchPredicate = elements.next();
//                    Iterator<String> fieldNames = searchPredicate.fieldNames();
//                    while (fieldNames.hasNext()) {
//                        String field = fieldNames.next();
//                        responseParams.put(field, searchPredicate.get(field).textValue());
...
//        Map<String, SearchCriteria> searchCriterias = new HashMap<String, SearchCriteria>();
//        for (String search : searchDTO.getSpecifications()) {
//            Pattern pattern = Pattern.compile("(.+?)(:|<|>)(.+?),");
//            Matcher matcher = pattern.matcher(search + ",");
//            while (matcher.find()) {
//                SearchCriteria criteria = new SearchCriteria(matcher.group(1), matcher.group(2), matcher.group(3));
//                searchCriterias.put(criteria.getKey(), criteria);
//            }
//        }
//        for (String key : searchCriterias.keySet()) {
//            SearchCriteria searchCriteria = searchCriterias.get(key);
//            Optional.ofNullable(searchCriteria.getValue()).ifPresent(o -> {
